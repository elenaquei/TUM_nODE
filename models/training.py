            ...
            if not self.turnpike:  ## Classical empirical risk minimization
                ...
                if self.threshold > 0:  # l1 controls
                    ...
                    if self.fixed_projector:  # maybe not needed
                        xd = torch.tensor(
                            [[6.0 / 0.8156, 0.5 / (2 * 0.4525)] if x == 1 else [-6.0 / 0.8156, -2.0 / (2 * 0.4525)] for
                             x in y_batch])
                        loss = self.loss_func(y_pred, y_batch.float()) + sum([self.loss_func(traj[k], xd)
                                                                              + self.loss_func(traj[k + 1], xd) for k in
                                                                              range(time_steps - 1)])
                    else:
    ...
    def _train_epoch(self, data_loader, epoch):
        ...
        for i, (x_batch, y_batch) in enumerate(data_loader):
            ...
            if self.is_resnet:
                ...
                time_steps = self.model.n_layers
                T = self.model.time_interval[1]
                dt = T / time_steps
