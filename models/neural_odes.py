...
class Dynamics(nn.Module):
    ...
    def forward(self, t, x):
        ...
        if self.architecture < 1:
            ...
            # Domenec Test
            out1 = torch.sqrt(
                self.non_linearity(x.matmul(w1_t.t()) + torch.ones(self.hidden_dim) + b1_t) + 1e-6 * torch.ones(
                    self.hidden_dim)) - torch.sqrt(1e-6 * torch.ones(self.hidden_dim))
            out2 = torch.sqrt(
                self.non_linearity(-x.matmul(w1_t.t()) + torch.ones(self.hidden_dim) + b1_t) + 1e-6 * torch.ones(
                    self.hidden_dim)) - torch.sqrt(1e-6 * torch.ones(self.hidden_dim))
    ...
    def __str__(self):
        ...
        if self.architecture < 1:
            if self.architecture == -1:
                string += str('w(t)' + activation_string[0] + '(x(t))+b(t)    over the interval t = [0, ' + str(
                    self.T) + '],\n\n')
            else:
                string += str(
                    activation_string[0] + '(w(t)x(t)+b(t))    over the interval t = [0, ' + str(self.T) + '],\n\n')
            for k in range(self.time_steps):
                string += str(
                    'W[' + str(k) + '] = ' + str(self.fc2_time[k].weight.detach().numpy()) + ',        b[' + str(
                        k) + '] = ' + str(self.fc2_time[k].bias.detach().numpy()) + '\n\n')
        else:
            string += str(
                'w1(t)' + activation_string[0] + '(w2(t)x(t)+b2(t))+b1(t)    over the interval t = [0, ' + str(
                    self.T) + '],\n\n')
            for k in range(self.time_steps):
                string += str(
                    'W1[' + str(k) + '] = ' + str(self.fc1_time[k].weight.detach().numpy()) + ',        b1[' + str(
                        k) + '] = ' + str(self.fc1_time[k].bias.detach().numpy()) + '\n\n')
                string += str(
                    'W2[' + str(k) + '] = ' + str(self.fc3_time[k].weight.detach().numpy()) + ',        b2[' + str(
                        k) + '] = ' + str(self.fc3_time[k].bias.detach().numpy()) + '\n\n')
        return string

    def print_to_file(self, filename, open_style = "w"):
        activation_string = [i for i in activations if activations[i] == self.non_linearity]
        ...
        if self.architecture < 1:
            ...
            for k in range(self.time_steps):
                ...
                string += str('W2[' + str(k) + '] = ' + str(self.fc3_time[k].weight.detach().numpy()) + '\nb2[' + str(
                    k) + '] = ' + str(self.fc3_time[k].bias.detach().numpy()) + '\n\n')
        with open(filename, open_style) as text_file:
...
class Dynamics_reduced(nn.Module):
    ...
    def __str__(self):
        ...
        if self.architecture < 1:
            if self.architecture == -1:
                string += str(
                    'w(t)' + activation_string[0] + '(x(t))+b(t)    over the interval t = [0, ' + self.T + ']\n')
            else:
                string += str(activation_string[0] + '(w(t)x(t)+b(t))\n')
            for k in range(self.time_steps):
                string += str(
...
class adj_Dynamics(nn.Module):
    ...
    def __init__(self, dynamics, x_traj, device, data_dim, hidden_dim, architecture='outside', augment_dim=0,
...
class NeuralODE(nn.Module):
    ...
    ***
    ...
    def __init__(self, device, data_dim, hidden_dim, output_dim=2,
        ...
        if reduced_dynamics:
            dynamics = Dynamics_reduced(device, data_dim, hidden_dim, augment_dim, non_linearity, architecture, self.T,
                                        self.time_steps)
        else:
    ...
    def print_to_file(self, filename, open_style = "w"):
        self.flow.dynamics.print_to_file(filename, open_style)
        return


class convolutionalNeuralODE(nn.Module):
    """
    A new neural network in which a neuralODE is sandwiched between two convolutional linear layers.
    The goal is to have flexibility in the size of the neuralODE.
    
    ***
    - output dim is an int the dimension of the labels.
    - architecture is a string designating the structure of the dynamics f(x,u)
    - hidden_dim is the dimension of the neuralODE
    ***
    """

    def __init__(self, device, data_dim, hidden_dim, output_dim=2,
                 non_linearity='tanh',
                 tol=1e-3, architecture='inside',
                 T=10, time_steps=10):
        super(convolutionalNeuralODE, self).__init__()
        self.device = device
        self.data_dim = data_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
        ...
        augment_dim = 0
        self.time_steps = time_steps
        dynamics = Dynamics(device, data_dim, hidden_dim, augment_dim, non_linearity, architecture, T, time_steps)
        adjoint = False
        flow = Semiflow(device, dynamics, tol, adjoint, T, time_steps)
        self.flow = flow
        self.start_linear_layer = nn.Linear(self.data_dim,
                                            self.hidden_dim)

        self.end_linear_layer = nn.Linear(self.hidden_dim,
                                          self.output_dim)
        self.non_linearity = nn.Tanh()

    def forward(self, x):
        x = self.start_linear_layer(x)

        features = self.flow(x)
        traj = self.flow.trajectory(x, self.time_steps)

        out = self.end_linear_layer(features)
        out_traj = self.end_linear_layer(traj)

        return out, out_traj

    def __str__(self):
        string = str('W = ' + str(self.start_linear_layer.weight.detach().numpy()) + '\nb = ' +
                     str(self.start_linear_layer.bias.detach().numpy()) + '\n\n')
        string += str(self.flow.dynamics)
        string += str('W = ' + str(self.end_linear_layer.weight.detach().numpy()) + '\nb = ' +
                      str(self.end_linear_layer.bias.detach().numpy()) + '\n\n')
        return string
    ...
    def print_to_file(self, filename, open_style="w"):
        ...
        with open(filename, open_style) as text_file:
            text_file.write(string)
        self.flow.dynamics.print_to_file(filename,"a")
        string = str('W = ' + str(self.end_linear_layer.weight.detach().numpy()) + '\nb = ' +
                     str(self.end_linear_layer.bias.detach().numpy()) + '\n\n')
...
class robNeuralODE(nn.Module):
    ...
    ***
    ...
    def __init__(self, device, data_dim, hidden_dim, output_dim=2,
        ...
        self.f_dynamics = Dynamics(device, data_dim, hidden_dim, augment_dim, non_linearity, architecture, self.T,
                                   self.time_steps)
    ...
    def forward(self, x, return_features=False):
        ...
        p1 = torch.zeros(x.size()) + torch.tensor(
            [1, 0])  # we want to take initial conditions in all canonical directions in to account
