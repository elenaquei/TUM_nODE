...
class Dynamics(nn.Module):
    ...
    def derivative(self, t, x):
        ...
        if self.architecture < 1:
            ...
            if self.architecture < 0:
                ...
                #self.non_linear_derivative(w_t).matmul(torch.diag(x))
            else:
                # \sigma(w(t)x(t)+b(t))   outer
                # # #     -> derivative is \sigma'(w(t)x(t) + b(t)) \row dy row kronecked product w(t)
    ...
    def __str__(self):
        ...
        if self.architecture < 1:
            if self.architecture == -1:
                string += str(
                    'w(t)' + activation_string[0] + '(x(t))+b(t)    over the interval t = [0, ' + str(self.T) + '],n\n')
            else:
                string += str(
